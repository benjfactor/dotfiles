...
- The `userConfig` schema shape for R10, which stage 2 needs and stage 1 does not. Four facets of it surfaced while executing stage 1 and are recorded rather than settled:
  - **Whether map-valued config is an option or a file.** R10 commits to moving `TEAM_CHANNELS` behind `userConfig`, which quietly assumes a settings option can hold a map of arbitrary length. If it cannot, or if maintaining a nine-entry object in `settings.json` is unpleasant, the alternative for that map and for `PEOPLE` is a user-maintained data file beside the credentials, with the plugin shipping an empty default. This decides whether R10 is a settings change or a file-format change, so it is the first of the four to answer.
  - **Whether R10's enumeration should cover the OAuth app identity.** R10 names the Chat space ID, the `@vendasta/meerkats` handle and `TEAM_CHANNELS`. It does not name `CLIENT_ID`, the secret name `google-chat-oauth-client-secret`, or the project `repcore-prod` in `send_gchat.py`, which together express one decision — which OAuth app this is. None of the three is a credential, so R35's sensitive-option path does not apply. They qualify on substitutability instead: a consumer outside the org cannot use that client ID at all, and unlike `TEAM_CHANNELS` there is no live-lookup fallback to degrade to, so leaving them literal blocks publication harder than anything R10 does name. Probably one config group rather than three options that can be set inconsistently.
  - **Whether `PEOPLE` belongs in R10's scope.** It maps person slugs to Google Chat user IDs for `--mention`, and currently carries three colleagues' real names and IDs in a public repository. No stage gate claims it. A consumer's colleagues are different people entirely, so it is user data rather than a setting — the same shape question as `TEAM_CHANNELS` above. It predates this plan; stage 1 only moved the file.
  - **That `CREDS_FILE` should be derived, not configured.** It does not qualify under R10 — the path is already `$HOME`-relative and carries no org name, so it publishes fine as-is. But once the OAuth app identity becomes configurable, a fixed cache path becomes a latent bug: the cached refresh token belongs to whichever app minted it, so changing the app leaves a stale cache that fails auth confusingly instead of re-prompting. Exposing the path as a second, independently-settable option makes that worse by letting two knobs disagree; deriving it from the app identity so the cache cannot mismatch is the candidate fix. Two asides while in there: it hardcodes `~/.config` rather than honouring `$XDG_CONFIG_HOME`, and the path itself is not sensitive even though its contents are — already handled by the file living outside the repo.
